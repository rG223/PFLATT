...
class BasicBlock(nn.Module):
    ...
    def __init__(
        ...
        self.stride = stride
        # self.scse = scSE(in_channels=planes, mode=['cSE', 'sSE'])
...
class ResNet(nn.Module):
    ...
    def _make_layer(self, block: Type[Union[BasicBlock, Bottleneck, BasicBlock_attention]], planes: int, blocks: int,
...
class ResNet_attention(nn.Module):
    ...
    def __init__(
        ...
        self.relu = nn.ReLU(inplace=True)
        self.maxpool = nn.MaxPool2d(kernel_size=3, stride=2, padding=1)
        self.layer1 = self._make_layer(block, 64, layers[0], layer=16)
        self.layer2 = self._make_layer(block, 128, layers[1], stride=2,
                                       dilate=replace_stride_with_dilation[0], layer=8)
        self.layer3 = self._make_layer(block, 256, layers[2], stride=2,
                                       dilate=replace_stride_with_dilation[1], layer=4)
        self.layer4 = self._make_layer(block, 512, layers[3], stride=2,
                                       dilate=replace_stride_with_dilation[2], layer=2)
        self.avgpool = nn.AdaptiveAvgPool2d((1, 1))
        self.fc = nn.Linear(512 * block.expansion, num_classes)
    ...
    def _make_layer(self, block: Type[Union[BasicBlock, Bottleneck, BasicBlock_attention]], planes: int, blocks: int,
                    stride: int = 1, dilate: bool = False, layer: int=0) -> nn.Sequential:
        norm_layer = self._norm_layer
        downsample = None
        previous_dilation = self.dilation
        if dilate:
        ...
        layers = []
        layers.append(block(self.inplanes, planes, stride, downsample, self.groups,
                            self.base_width, previous_dilation, norm_layer, layer))
        self.inplanes = planes * block.expansion
        for _ in range(1, blocks):
            layers.append(block(self.inplanes, planes, groups=self.groups,
                                base_width=self.base_width, dilation=self.dilation,
